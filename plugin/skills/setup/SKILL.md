...
{ "command": "docker",
  "args": ["run", "-i", "--rm", "-v", "kotlin-lib-mcp-cache:/home/mcp/.cache",
           "ghcr.io/aoreshkov/kotlin-lib-mcp:0.6"] }
```
...
| `manifest unknown` / pull error | Registry unreachable, or an air-gapped machine | `docker pull ghcr.io/aoreshkov/kotlin-lib-mcp:0.6` by hand and read the error |
| Tools present, every `fetch_library` fails | No network route to Maven Central from the container, or a corporate proxy | Test with `docker run --rm ghcr.io/aoreshkov/kotlin-lib-mcp:0.6 --help`; pass a mirror with `--repo <url>` |
